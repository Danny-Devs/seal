...
4. Build the admin-cli tool.

```bash
cargo run --bin admin-cli -- --help
```
...
cp crates/admin-cli/dkg.example.yaml dkg-state/dkg.yaml
```
...
```bash
cargo run --bin admin-cli -- publish-and-init
```
...
```bash
cargo run --bin admin-cli -- check-committee -c dkg-state/dkg.yaml
```
...
```bash
cargo run --bin admin-cli -- genkey-and-register \
  -u <YOUR_SERVER_URL> \
  -n <YOUR_SERVER_NAME>
```
...
```bash
cargo run --bin admin-cli -- create-message
```
...
cargo run --bin admin-cli -- process-all-and-propose
```
...
# in seal/
rm -rf dkg-state & mkdir dkg-state
cp crates/admin-cli/dkg-rotation.example.yaml dkg-state/dkg.yaml
```
...
```bash
cargo run --bin admin-cli -- init-rotation
```
...
```bash
cargo run --bin admin-cli -- create-message -o <MASTER_SHARE_VX>
```
...
```bash
cargo run --bin admin-cli -- init-state
```
