...
pub async fn fetch_committee_from_key_server(
    ...
    // Get package ID from object type.
    let object_type = ks_object
        .object_type
        .ok_or_else(|| anyhow!("Key server has no type"))?;
...
pub async fn fetch_upgrade_proposal(
    ...
    // Define the UpgradeManagerKey struct (empty struct for BCS serialization).
