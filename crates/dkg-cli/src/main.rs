...
use seal_committee::{
    ...
    fetch_committee_from_key_server, fetch_key_server_by_committee, fetch_upgrade_proposal,
    CommitteeState, KeyServerV2, Network, ServerType, UpgradeVote,
};
use serde::Serialize;
...
enum Commands {
    ...
    /// Vote for package upgrade (as committee member).
    Vote {
    ...
    /// Authorize and execute package upgrade (after quorum reached).
    AuthorizeAndUpgrade {
        /// Path to the Move package to upgrade to.
        #[arg(short, long, default_value = "move/committee")]
        package_path: PathBuf,
    ...
    /// Reset upgrade proposal (if threshold of rejections reached).
    ResetProposal {
        /// Key server object ID.
        #[arg(short, long)]
        key_server_id: String,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::PublishAndInit { config } => {
            ...
            // Find UpgradeCap (transferred to coordinator).
            ...
            // Find InitCap (created and transferred to coordinator).
        ...
        Commands::Vote {
            ...
            let digest = get_package_digest_bytes(&package_path, &network)?;
            println!("\nPackage digest: {}", Hex::encode_with_format(&digest));

            // Fetch key server to get committee ID.
            let mut grpc_client = create_grpc_client(&network)?;
            let key_server_obj_id = ObjectID::from_hex_literal(&key_server_id)?;
            let key_server_addr = Address::from(key_server_obj_id.into_bytes());
            let (committee_id, committee_pkg) =
                fetch_committee_from_key_server(&mut grpc_client, &key_server_addr).await?;

            println!("Committee ID: {}", committee_id);
            println!("Committee package: {}", committee_pkg);
            ...
            vote_builder.programmable_move_call(
                committee_pkg,
                "seal_committee".parse()?,
                "approve_digest_for_upgrade".parse()?,
                vec![],
                vec![committee_arg, digest_arg],
            );

            let (gas_price, gas_budget, gas_coin_ref) =
                get_gas_params(&mut grpc_client, &wallet, voter_address, cli.gas_budget).await?;
        ...
        Commands::AuthorizeAndUpgrade {
            ...
            let committee_arg_auth = upgrade_builder
                .obj(get_shared_committee_arg(&mut grpc_client, committee_obj_id, true).await?)?;
            let digest_arg = upgrade_builder.pure(digest.clone())?;
            ...
            let upgrade_ticket = upgrade_builder.programmable_move_call(
                ...
                vec![],
                vec![committee_arg_auth, digest_arg],
            );
            ...
            println!("\nExecuting authorize-and-upgrade transaction...");
        ...
        Commands::CheckKeyServerStatus {
            ...
            match fetch_upgrade_proposal(&mut grpc_client, &committee_id).await {
                Ok(Some(proposal)) => {
                    ...
                    println!("  Votes ({}):", proposal.votes.0.contents.len());
                    ...
                    let approvals = proposal
                        ...
                        .contents
                        .iter()
                        .filter(|entry| matches!(entry.value, UpgradeVote::Approve))
                        .count();
                    let rejections = proposal
                        ...
                        .contents
                        .iter()
                        .filter(|entry| matches!(entry.value, UpgradeVote::Reject))
                        .count();

                    println!("\n  Summary:");
                    println!("    Approvals: {}/{}", approvals, committee.threshold);
                    println!("    Rejections: {}/{}", rejections, committee.threshold);

                    if approvals >= committee.threshold as usize {
                        println!("    Status: ✓ Ready to authorize upgrade");
                    } else if rejections >= committee.threshold as usize {
                        println!("    Status: ✗ Ready to reset proposal");
                    } else {
                        println!(
                            "    Status: ⏳ Waiting for {} more vote(s)",
                            (committee.threshold as usize)
                                .saturating_sub(approvals.max(rejections))
                        );
                    }
                }
                Ok(None) => {
...
fn compute_package_digest(package_path: &Path, network: &Network) -> Result<()> {
    ...
    println!(
        ...
        package_path
            ...
            .unwrap_or_default()
            .to_string_lossy(),
        Hex::encode_with_format(&digest_bytes)
    );
